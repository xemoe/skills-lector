...
- `status`: read `.cheats/raw.json` (if present) and report how many prompts were
  last extracted and when — use `raw.prompts.length` for the count,
  `raw.extractedAt` for the timestamp, and `raw.typedCount` / `raw.legacyCount`
  for the provenance breakdown — without re-running anything.
