...
**ฮุก** ไม่ใช่ไฟล์ของตัวเอง — มันอยู่ในคีย์ \`hooks\` ของ \`settings.json\` และ \`settings.local.json\` (ขอบเขต \`local\`) ทั้งระดับ personal, plugin, และ project ส่วน **โพย** ไม่มีพาธเลย: มันถูกขุดจากทรานสคริปต์เซสชัน Claude Code ของคุณ และเก็บไว้ในที่เก็บ SQLite เล็ก ๆ ในเครื่องใต้ \`~/.skills-lector/\`
...
คุณสามารถชี้สแกนเนอร์ไปยังไดเรกทอรีเพิ่มเติมได้ด้วย \`skills-lector.config.json\` ที่วางคู่กับตำแหน่งที่รัน dev server หรือใช้ตัวแปรสภาพแวดล้อม \`SKILLS_SCAN_ROOTS\` ดูตำแหน่งทั้งหมดที่กำลังสแกนอยู่ได้ที่หน้า **Sources**`,
        ...
        catalogTour: {
            ...
            body: `Skills Lector มีมุมมองเหล่านี้ สร้างจากการสแกนชุดเดียวกันทั้งหมด ทุกอย่างอ่านจากดิสก์ของคุณ — มีเพียง **Presets** เท่านั้นที่เขียนกลับไปยังไฟล์สกิลและคำสั่งของคุณ (การทำเครื่องหมายโปรดให้โพยก็บันทึกเช่นกัน แต่ลงฐานข้อมูลในเครื่อง ไม่ใช่ไฟล์ \`.md\` ของคุณ)
